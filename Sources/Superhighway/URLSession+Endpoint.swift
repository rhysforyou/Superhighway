...
public extension URLSession {
  ...
  /// - Returns: A tuple of the decoded response data and HTTP response
  func response<Response>(for endpoint: Endpoint<Response>) async throws -> (
    Response, URLResponse
  ) {
    let request = endpoint.request
...
public extension URLSession {
  ...
  final class EndpointPublisher<Response>: Combine.Publisher {
  ...
  func endpointPublisher<Response>(
    _ endpoint: Endpoint<Response>
  ) -> EndpointPublisher<Response> {
    EndpointPublisher(endpoint: endpoint, session: self)
  }
}
